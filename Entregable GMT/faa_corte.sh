...
gmt begin cortefaa png
	...
	gmt grdimage $Map4 -I$hs_faa270 -Ccolorsfaa.cpt -Jd0/-70/-90/-49/1:50000000 -Bxa3g3 -Bya3g3 --FORMAT_GEO_MAP=dddF
	...
	gmt plot -R$Map4 -W2p,blue ridge.txt
	gmt plot -Sc0.25c -Gblue ridge.txt
	# Gener
	#ate cross-profiles 400 km long, spaced 10 km, samped every 2km
	# and stack these using the median, write stacked profile
	gmt grdtrack ridge.txt -G$Map4 -C400k/5k/30k+v -Sm+sstack.txt > table.txt
	gmt plot -W0.5p table.txt
	# Show upper/lower 2-sigma confidence bounds encountered as an envelope
	gmt plot -R-200/200/-200/200 -JX15c/7.5c -W3p stack.txt -i0,1,5,6 -L+b -Glightgray -Yh+3c
	gmt basemap  -Bxafg1000+l"Distance from ridge (km)" -Byaf+l"mGal" -BWSne
